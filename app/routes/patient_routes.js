...
const ipCheck = require("../../lib/pangea");
...
router.get("/patients/selected", requireToken, (req, res, next) => {
...
router.post("/patients/data", (req, res, next) => { 
  const pubKey = recoverPublicKey(req.body.signature, req.body.message);
  const address = publicKeyToAddress(pubKey); 

  ipCheck(req.body.message)
    .then(customErrors.handleForbiddenCountry)
    .then(() => {
        Patient.findOne({ account: address })
	  .populate("doctors")
    .then(handle404) 
    .then((patient) => {
      return encryptWithPublicKey(
        pubKey,
        JSON.stringify({ patient: patient.toObject() })
      );
    })
    .then((cypherInfo) => {
      res.status(200).json({ cypher: cypherInfo });
    })
    })
